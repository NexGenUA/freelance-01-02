$(document).ready(function($){
  ...
  $('input[name="value"]').on('focusout', function() {
    ...
    if(value === '') {
      ...
      if (+value.slice(0, -1) === 0 || +value === 0) {
        $(this).next('p').html('Value must be greater than 0');
        $(this).focus();
        $('#moyn-key').text('0.00');
        $(this).val('0.00');
        return false;
  ...
  $('select[name="payment-method"], select[name="payment-currency"]').on('change', function() {
    reserve();      
  })
  ...
  function reserve() {
    var payment_currency = $('select[name=payment-currency]').val();
    var payment_method = $('select[name=payment-method').val();
    var reserveUrl = 'http://prosgold.ru/reserve.php'
    $.ajax({
      type: "POST",  
      dataType: 'json',
      data: { payment_method, payment_currency },
      url: reserveUrl,
      success: function(data) {
        $('#reserve').text(data.Value);
      },
      error: function(err) {
        console.log(0);
      }
