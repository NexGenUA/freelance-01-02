$(document).ready(function($){
  ...
  function hashChange() {
    ...
    window.location.hash = 'exchange-' + from + '-to-' + method + '-' + to;
  }
  ...
  function setValues() {
    ...
    var from = hash[1];
    var method = length === 5 ? hash[3][0].toUpperCase() + hash[3].slice(1)
      : hash[3][0].toUpperCase() + hash[3].slice(1)
      + ' ' + hash[4][0].toUpperCase() + hash[4].slice(1);
    method = method === 'Dash' ? 'DASH' : method;
    var to = hash[length - 1];
    ...
    for (var key in validate) {
      if (key === method 
        ...
        if (/dash|ltc|btc/i.test(to)) $('select[name="payment-method"]').trigger('change');
